...
class Engine:
    def __init__(self, session: Session, browser_headers = None):
        self._browser_headers = browser_headers if browser_headers is not None else HEADERS
    ...
    def request(self, r):
        ...
        resp = requests.request(req.method, req.url, data=req.payload, headers=headers)
        if not resp.ok:
            print(req.url)
            print(resp)
            print(resp.text)
