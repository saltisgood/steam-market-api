...
class ItemOrdersHistogramResponse(Response):
    ...
    def __str__(self):
        top_buys = ', '.join(str(pq) for pq in self.buys[:self.TOP_ORDER_COUNT])
...
class ItemPricingResponse(Response):
    ...
    def __str__(self):
        return 'ItemPricingResponse{{lowest_price={}, volume={}, median_price={}}}'.format(self.lowest_price, self.volume, self.median_price)
...
class MedianSaleHistory:
    def __init__(self, time, price, quantity):
        if type(time) is str:
            ...
            self.time = time
        self.price = Price(price, 'USD') # The price is in USD unless you have the steamLoginSecure cookie set
