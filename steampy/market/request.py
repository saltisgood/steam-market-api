...
from .response import ItemNameIdResponse, ItemOrdersHistogramResponse, ItemPricingResponse, SalesHistoryResponse, CreateListingResponse, RemoveListingResponse
...
class ItemNameId:
    ...
    def build_request(self, session):
        return Request(self.BASE_URL.format(appid=self.appid, item=self.item_name))
...
class ItemOrdersHistogram:
    ...
    def build_request(self, session):
        return Request(self.BASE_URL.format(country=self.country, language=self.language, currency=self.currency, item_nameid=self.item_nameid))
...
class ItemPricing:
    ...
    def build_request(self):
        return Request(self.BASE_URL.format(appid=self.appid, currency=self.currency, item=self.item_name))
...
class CreateListing:
    ...
    def build_request(self, session):
        return Request(method='POST',
            url=self.BASE_URL,
            payload=self.BASE_URL.format(session_id=session.session_id, appid=self.appid, context_id=self.context_id, asset_id=self.asset_id, amount=self.amount, price=self.price.to_network_format()),
...
class RemoveListing:
    ...
    def build_request(self, session):
        return Request(method='POST',
            url=self.BASE_URL.format(listing=self.listing))
